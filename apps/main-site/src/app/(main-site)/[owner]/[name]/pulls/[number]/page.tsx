import { Link } from "@packages/ui/components/link";
import { Skeleton } from "@packages/ui/components/skeleton";
import { Suspense } from "react";
import { serverQueries } from "@/lib/server-queries";
import {
	FilesChangedClient,
	PullRequestDetailClient,
} from "./pr-detail-client";

// ---------------------------------------------------------------------------
// Server component — fetches initial data, wraps client in Suspense
// ---------------------------------------------------------------------------

export default async function PullRequestDetailPage(props: {
	params: Promise<{ owner: string; name: string; number: string }>;
}) {
	const params = await props.params;
	const { owner, name } = params;
	const prNumber = parseInt(params.number, 10);

	// Start fetching immediately — do NOT await. Pass the promise to the client
	// component so React Suspense can handle the loading state.
	const prPromise = serverQueries.getPullRequestDetail.queryPromise({
		ownerLogin: owner,
		name,
		number: prNumber,
	});

	const filesPromise = serverQueries.listPrFiles.queryPromise({
		ownerLogin: owner,
		name,
		number: prNumber,
	});

	return (
		<div className="max-w-4xl">
			<div className="mb-6">
				<Link
					href={`/${owner}/${name}/pulls`}
					className="text-sm text-muted-foreground hover:text-foreground"
				>
					&larr; Pull Requests
				</Link>
			</div>

			<Suspense fallback={<DetailSkeleton />}>
				<PullRequestDetailClient
					owner={owner}
					name={name}
					prNumber={prNumber}
					initialDataPromise={prPromise}
				/>
			</Suspense>

			<Suspense fallback={<FilesSkeleton />}>
				<FilesChangedClient
					owner={owner}
					name={name}
					prNumber={prNumber}
					initialDataPromise={filesPromise}
				/>
			</Suspense>
		</div>
	);
}

// ---------------------------------------------------------------------------
// Skeleton components (server-rendered)
// ---------------------------------------------------------------------------

function DetailSkeleton() {
	return (
		<div>
			<Skeleton className="h-8 w-3/4" />
			<Skeleton className="mt-3 h-5 w-1/2" />
			<Skeleton className="mt-6 h-40 w-full" />
			<Skeleton className="mt-8 h-6 w-32" />
			<div className="mt-4 space-y-4">
				<Skeleton className="h-24 w-full" />
				<Skeleton className="h-24 w-full" />
			</div>
		</div>
	);
}

function FilesSkeleton() {
	return (
		<div className="mt-8">
			<Skeleton className="h-6 w-40 mb-4" />
			<Skeleton className="h-64 w-full" />
		</div>
	);
}
